// 本文件为 SQLite 集成测试——Select/SelectRaw/Distinct/子查询列选择。
...
// TestSQLiteInteg_SelectSub 验证 SELECT 子句中的子查询。
func TestSQLiteInteg_SelectSub(t *testing.T) {
	db := openSQLiteTestDB(t)
	setupSQLiteUsersTable(t, db)
...
// TestSQLiteInteg_SelectReplacesColumns
// Select 为替换语义、无状态残留——后续 Select 覆盖前列，Select("*") 恢复全列。
func TestSQLiteInteg_SelectReplacesColumns(t *testing.T) {
	db := openSQLiteTestDB(t)
	setupSQLiteUsersTable(t, db)
...
// TestSQLiteInteg_SubSelectResetBindings
// 子查询中 Select("*") 为替换语义，无列/绑定残留。
func TestSQLiteInteg_SubSelectResetBindings(t *testing.T) {
	db := openSQLiteTestDB(t)
	setupSQLiteUsersTable(t, db)
	setupSQLiteOrdersTable(t, db)
...
// TestSQLiteInteg_JsonSelectWhereOrder 验证 JSON 提取在 select/where/orderBy
// 中通过 SelectRaw/WhereRaw/OrderByRaw 组合（json_extract，含路径引号转义）。
func TestSQLiteInteg_JsonSelectWhereOrder(t *testing.T) {
	db := openSQLiteTestDB(t)
...
// TestSQLiteInteg_NewApi_AddSelect 验证 AddSelect 追加列（去重）与 SelectSub 子查询列。
func TestSQLiteInteg_NewApi_AddSelect(t *testing.T) {
	...
	if len(rows) != 5 {
		t.Fatalf("SelectSub: expected 5 rows, got %d", len(rows))
	}
	for i, want := range expectedCounts {
		if rows[i].OrderCount != want {
			t.Errorf("SelectSub row %d: expected %d, got %d", i, want, rows[i].OrderCount)
		}
	}
}
