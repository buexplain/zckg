...
| RIGHT JOIN | 支持 | 支持 | 3.39 以下不支持（框架不做版本检测，由数据库报错） |
...
| `ErrNoFields` | 结构体无可参与写入的字段（无可导出字段，或全部字段为 nil/不可用被跳过） |
| `ErrEmptyData` | 插入数据为空 |
...
| `ErrInsertUsingColumnMismatch` | InsertUsing/InsertOrIgnoreUsing 的目标列数与子查询 SELECT 列数不一致（子查询列数可静态判定时） |
