...
// TestCheckUnsupportedDefaults_EmbeddedPtrNoWarning 锁死问题2：
// 导出匿名嵌入指针结构体的值类型/指针类型 default 字段均不应触发「never applied」误告警，
// 且注册期 applyDefaults 实际填充了默认值（告警与运行时行为一致）。
func TestCheckUnsupportedDefaults_EmbeddedPtrNoWarning(t *testing.T) {
	var buf bytes.Buffer
	old := slog.Default()
	slog.SetDefault(slog.New(slog.NewTextHandler(&buf, nil)))
	defer slog.SetDefault(old)
	...
	type EmbedBase struct {
		Page   int     `json:"page" default:"5"`    // 值类型 default
		Status *string `json:"status" default:"ok"` // 指针类型 default
	}
	type req struct {
		*EmbedBase
	...
	checkUnsupportedDefaults(reflect.TypeOf(req{}), true, true, "POST", "/t", "handler", "file.go", 1, make(map[reflect.Type]bool))
	if out := buf.String(); out != "" {
		t.Fatalf("embedded pointer struct fields are defaults-reachable, expected no warning, got: %s", out)
	}

	// 注册期 applyDefaults 应实际填充嵌入结构体的默认值（与无告警结论一致）
	fn := func(_ context.Context, _ req) (embedDefaultRes, error) { return embedDefaultRes{}, nil }
	entry, err := buildEntry(fn, nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	tpl := entry.defaultReq.Interface().(req)
	if tpl.EmbedBase == nil {
		t.Fatal("embedded pointer should be materialized by applyDefaults")
	}
	if tpl.Page != 5 {
		t.Fatalf("page = %d, want default 5", tpl.Page)
	}
	if tpl.Status == nil || *tpl.Status != "ok" {
		t.Fatalf("status should be filled with 'ok', got %v", tpl.Status)
	}
}

type embedDefaultRes struct {
	OK bool `json:"ok"`
}

// TestHasRequestPhaseDefaults_UnexportedValueEmbed 锁死：未导出「值」嵌入内部的指针 default
// 字段（经切片等动态容器）必须被识别为请求阶段需补填（与 buildStructMeta 的展开逻辑对齐）。
func TestHasRequestPhaseDefaults_UnexportedValueEmbed(t *testing.T) {
	type item struct {
		Qty *int `json:"qty" default:"1"`
	}
	...
	if !hasRequestPhaseDefaults(reflect.TypeOf(req{}), nil) {
		t.Fatal("hasRequestPhaseDefaults should be true for pointer default inside unexported value embed's slice")
	}
}

// TestCheckUnsupportedDefaults_UnexportedValueEmbed 锁死：未导出「值」嵌入内部的 default 字段
// 被正确扫描——值/指针 default 不产生误告警（值嵌入可达），不支持类型 default 仍告警。
func TestCheckUnsupportedDefaults_UnexportedValueEmbed(t *testing.T) {
	var buf bytes.Buffer
	old := slog.Default()
	slog.SetDefault(slog.New(slog.NewTextHandler(&buf, nil)))
	defer slog.SetDefault(old)
	...
	checkUnsupportedDefaults(reflect.TypeOf(req{}), true, true, "POST", "/t", "handler", "file.go", 1, make(map[reflect.Type]bool))
	if out := buf.String(); out != "" {
		t.Fatalf("unexported value embed is defaults-reachable, expected no warning, got: %s", out)
	}

	// 不支持类型（map）的 default 在未导出值嵌入内部仍应被正确告警（而非静默跳过）
	type unexportedBad struct {
		Raw map[string]any `json:"raw" default:"x"`
	}
	type reqBad struct {
		unexportedBad
	}
	checkUnsupportedDefaults(reflect.TypeOf(reqBad{}), true, true, "POST", "/bad", "handler", "file.go", 1, make(map[reflect.Type]bool))
	out := buf.String()
	if !strings.Contains(out, "unsupported type") {
		t.Fatalf("unsupported default type inside unexported value embed should warn 'unsupported type', got: %s", out)
	}
}
