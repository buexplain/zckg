// 本文件为 MySQL 方言单元测试——Builder 基础能力（方言特有的通用行为）。
...
// TestMySQLGrammar_WrapTable_MixedCaseAlias 验证 WrapTable 的 " as " 拆分对大小写不敏感：
// "orders As o"、"orders aS o" 等混合大小写别名均被正确拆分包裹，而非整体当作一个标识符。
func TestMySQLGrammar_WrapTable_MixedCaseAlias(t *testing.T) {
	g := NewMySQLGrammar()
	tests := []struct {
		in   string
		want string
