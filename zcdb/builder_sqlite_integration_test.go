...
// TestSQLiteInteg_EmbeddedShadowScanOuterWins 验证扫描列映射的「外层优先」：
// 外层字段与嵌入字段同名列时，值写入外层字段，嵌入字段保持零值。
func TestSQLiteInteg_EmbeddedShadowScanOuterWins(t *testing.T) {
	db := openSQLiteTestDB(t)
	setupSQLiteUsersTable(t, db)
...
// TestSQLiteInteg_EmbeddedShadowInsertOuterWins 验证 Insert 字段提取的「外层优先」：
// 同名列去重后仅保留外层字段，只写一列且值取自外层。
func TestSQLiteInteg_EmbeddedShadowInsertOuterWins(t *testing.T) {
	db := openSQLiteTestDB(t)
	setupSQLiteUsersTable(t, db)
...
// TestSQLiteInteg_WhereInExpression 验证 WhereIn/WhereNotIn 的 Expression 元素直接内嵌、
// 不占占位符，混合普通绑定值正确执行。
func TestSQLiteInteg_WhereInExpression(t *testing.T) {
	db := openSQLiteTestDB(t)
	setupSQLiteUsersTable(t, db)
...
// TestSQLiteInteg_HavingNull 验证 Having 传 nil 的 IS NULL / IS NOT NULL 语义真实执行。
func TestSQLiteInteg_HavingNull(t *testing.T) {
	db := openSQLiteTestDB(t)
	setupSQLiteUsersTable(t, db)
...
// TestSQLiteInteg_ExistsStateRestore 验证 Exists 编译期临时改动在返回后完整恢复，
// 同一 Builder 复用仍按原 Select/Limit/OrderBy 查询。
func TestSQLiteInteg_ExistsStateRestore(t *testing.T) {
	db := openSQLiteTestDB(t)
	setupSQLiteUsersTable(t, db)
