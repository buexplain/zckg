...
func TestBuilder_ToMethodsEmptyTable(t *testing.T) {
	...
	t.Run("ToInsert", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).ToInsert(userInsert{Name: "a"})
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("ToInsertOrIgnore", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).ToInsertOrIgnore(userInsert{Name: "a"})
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("ToUpsert", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).ToUpsert(userInsert{Name: "a"}, []string{"id"}, nil)
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("ToUpdate", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).ToUpdate(userInsert{Name: "a"})
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("ToInsertOrIgnoreUsing", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).ToInsertOrIgnoreUsing([]string{"a"}, func(sub *Builder) {})
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("ToIncrement", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).ToIncrement([]string{"age"}, []any{1})
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("ToDecrement", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).ToDecrement([]string{"age"}, []any{1})
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
...
func TestBuilder_ToInsertUsingSubErrors(t *testing.T) {
	...
	t.Run("sub_err", func(t *testing.T) {
		b := newTestBuilder(g, nil).Table("dst")
...
func TestBuilder_ToUpsertUniqueByRequired(t *testing.T) {
	for _, g := range []Grammar{NewPostgresGrammar(), NewSQLiteGrammar()} {
		_, _, err := newTestBuilder(g, nil).Table("users").ToUpsert(userInsert{Name: "a"}, nil, nil)
...
func TestBuilder_ToAggregateInvalid(t *testing.T) {
	g := NewMySQLGrammar()
	_, _, err := newTestBuilder(g, nil).Table("users").ToAggregate("MEDIAN", "id")
...
func TestBuilder_ToIncrementInvalidArgs(t *testing.T) {
	g := NewMySQLGrammar()
	if _, _, err := newTestBuilder(g, nil).Table("users").ToIncrement(nil, nil); !errors.Is(err, ErrIncrementColumns) {
		t.Fatalf("expected ErrIncrementColumns for empty columns, got %v", err)
	}
	if _, _, err := newTestBuilder(g, nil).Table("users").ToIncrement([]string{"a"}, nil); !errors.Is(err, ErrIncrementColumns) {
...
func TestBuilder_HavingInvalidOp(t *testing.T) {
	...
	t.Run("op_not_string", func(t *testing.T) {
		b := newTestBuilder(g, nil).Table("orders").GroupBy("status").Having("cnt", 123, 456)
		if _, _, err := b.ToSelect(); !errors.Is(err, ErrInvalidOperator) {
			t.Fatalf("expected ErrInvalidOperator, got %v", err)
		}
	})
	t.Run("invalid_operator", func(t *testing.T) {
		b := newTestBuilder(g, nil).Table("orders").GroupBy("status").Having("cnt", "INVALID", 456)
		if _, _, err := b.ToSelect(); !errors.Is(err, ErrInvalidOperator) {
			t.Fatalf("expected ErrInvalidOperator, got %v", err)
		}
	})
...
func TestBuilder_JoinSubInvalidOp(t *testing.T) {
	g := NewMySQLGrammar()
	sub := newTestBuilder(g, nil).Table("logs").Select("user_id")
	b := newTestBuilder(g, nil).Table("users").JoinSub(sub, "l", func(jb *JoinBuilder) {
...
func TestBuilder_TerminalMethodsPropagateCompileError(t *testing.T) {
	...
	t.Run("First", func(t *testing.T) {
		var r row
		if err := newTestBuilder(g, nil).First(ctx, &r); !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("PluckSlice", func(t *testing.T) {
		var ids []int64
		if err := newTestBuilder(g, nil).Pluck(ctx, &ids, "id"); !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("PluckKeyBy", func(t *testing.T) {
		m := map[int64]row{}
		if err := newTestBuilder(g, nil).Pluck(ctx, &m, "id"); !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("Count", func(t *testing.T) {
		if _, err := newTestBuilder(g, nil).Count(ctx); !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("Exists", func(t *testing.T) {
		if _, err := newTestBuilder(g, nil).Exists(ctx); !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("Max", func(t *testing.T) {
		if _, err := newTestBuilder(g, nil).Max(ctx, "age"); !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("Value", func(t *testing.T) {
		var v int64
		if err := newTestBuilder(g, nil).Select("id").Value(ctx, &v); !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", err)
		}
	})
	t.Run("Cursor", func(t *testing.T) {
		var r row
		var got error
		for err := range newTestBuilder(g, nil).Cursor(ctx, &r) {
			got = err
		}
	...
	t.Run("CursorBy", func(t *testing.T) {
		var r row
		var got error
		for err := range newTestBuilder(g, nil).CursorBy(ctx, &r, 10, "id") {
			got = err
		}
		if !errors.Is(got, ErrEmptyTable) {
			t.Fatalf("expected ErrEmptyTable, got %v", got)
...
func TestBuilder_ExecMethodCompileErrors(t *testing.T) {
	...
	t.Run("InsertUsing", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Table("dst").InsertUsing(ctx, []string{"name"}, func(sub *Builder) {})
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("InsertUsing 子查询缺表应报 ErrEmptyTable，实际 %v", err)
		}
	})
	t.Run("InsertOrIgnoreUsing", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Table("dst").InsertOrIgnoreUsing(ctx, []string{"name"}, func(sub *Builder) {})
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("InsertOrIgnoreUsing 子查询缺表应报 ErrEmptyTable，实际 %v", err)
		}
	})
	t.Run("IncrementEmptyTable", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Force().Increment(ctx, "wallet", 1)
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("Increment 缺表应报 ErrEmptyTable，实际 %v", err)
		}
	})
	t.Run("DecrementOddExtra", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Decrement(ctx, "wallet", 1, "extra")
	...
	t.Run("DecrementEmptyTable", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Force().Decrement(ctx, "wallet", 1)
		if !errors.Is(err, ErrEmptyTable) {
			t.Fatalf("Decrement 缺表应报 ErrEmptyTable，实际 %v", err)
		}
	})
	t.Run("DeleteJoinNoJoin", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Force().Table("t").DeleteJoin(ctx)
...
func TestBuilder_ExtractErrorPropagation(t *testing.T) {
	...
	t.Run("InsertNil", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Table("users").Insert(ctx, nil)
	...
	t.Run("InsertSliceFirstNilPtr", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Table("users").Insert(ctx, []*userInsert{nil})
		if !errors.Is(err, ErrInvalidStruct) {
			t.Fatalf("expected ErrInvalidStruct, got %v", err)
		}
	})
	t.Run("InsertSliceMiddleNilPtr", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Table("users").Insert(ctx, []*userInsert{{Name: "a"}, nil})
		if !errors.Is(err, ErrInvalidStruct) {
			t.Fatalf("expected ErrInvalidStruct, got %v", err)
		}
	})
	t.Run("UpdateNil", func(t *testing.T) {
		_, err := newTestBuilder(g, nil).Table("users").Force().Update(ctx, nil)
		if !errors.Is(err, ErrInvalidStruct) {
			t.Fatalf("expected ErrInvalidStruct, got %v", err)
		}
	})
	t.Run("UpdateNoFields", func(t *testing.T) {
		...
		_, err := newTestBuilder(g, nil).Table("users").Force().Update(ctx, noFields{hidden: "x"})
...
func TestBuilder_NilEmbeddedPointerSkips(t *testing.T) {
	...
	t.Run("InsertSingleRowNilEmbed", func(t *testing.T) {
		...
		_, _, err := newTestBuilder(g, nil).Table("t").ToInsert([]EmbRow{{Name: "x"}})
		if err != nil {
	...
	t.Run("InsertLaterRowNilEmbed", func(t *testing.T) {
		...
		_, _, err := newTestBuilder(g, nil).Table("t").ToInsert([]EmbRow{
	...
	t.Run("UpdateNilEmbed", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).Table("t").ToUpdate(EmbRow{Name: "x"})
		if err != nil {
	...
	t.Run("InsertNilPtrFieldFirstRow", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).Table("t").ToInsert([]nilPtrRow{{Name: "x"}})
		if err != nil {
			t.Fatalf("首行 nil 指针字段应跳过列: %v", err)
		}
	})
	t.Run("InsertNilPtrFieldLaterRow", func(t *testing.T) {
		_, _, err := newTestBuilder(g, nil).Table("t").ToInsert([]nilPtrRow{
			{P: strPtr("a"), Name: "x"},
			{Name: "y"},
		})
		if err != nil {
